...
static std::string make_temp_upload_dir() {
    char pattern[] = "/tmp/netcore_upload_XXXXXX";
...
static void cleanup_upload_dir(const std::string& dir, const std::vector<std::string>& files) {
    for (size_t i = 0; i < files.size(); ++i)
        ::unlink((dir + "/" + files[i]).c_str());
    ::rmdir(dir.c_str());
}
...
int main() {
    ...
    std::string upload_dir = make_temp_upload_dir();
    types::__location upload_location;
    upload_location.upload_location = upload_dir;
    upload_location.content.allowed_methods = types::POST;
    ...
    const std::string multipart_body =
        ...
        "Hello upload\r\n"
        "------NetCoreBoundary--\r\n";
    std::stringstream multipart_len;
    multipart_len << multipart_body.size();
    const std::string multipart_head =
        "POST /upload HTTP/1.1\r\n"
        "Host: example.com\r\n"
        "Content-Type: multipart/form-data; boundary=----NetCoreBoundary\r\n"
        "Content-Length: " + multipart_len.str() + "\r\n"
        "\r\n";
    std::pair<types::HttpStatus, Request> multipart_req = parse_with_connection(multipart_head, multipart_body);
    Response::_http_response multipart_res = Response::make_response(multipart_req, upload_location, 8080);
    ok = expect_eq("upload multipart status", multipart_res._status, static_cast<uint16_t>(201)) && ok;
    ok = expect_eq("upload multipart location", multipart_res._headers["Location"], "/upload/sample.txt") && ok;
    ok = expect_eq("upload multipart file body", read_file_content(upload_dir + "/sample.txt"), "Hello upload") && ok;

    const std::string raw_body = "raw-data";
    std::stringstream raw_len;
    raw_len << raw_body.size();
    const std::string raw_head =
        "POST /upload HTTP/1.1\r\n"
        "Host: example.com\r\n"
        "Content-Length: " + raw_len.str() + "\r\n"
        "\r\n";
    std::pair<types::HttpStatus, Request> raw_req = parse_with_connection(raw_head, raw_body);
    Response::_http_response raw_res = Response::make_response(raw_req, upload_location, 8080);
    ok = expect_eq("upload raw status", raw_res._status, static_cast<uint16_t>(201)) && ok;
    ok = expect_contains("upload raw location", raw_res._headers["Location"], "/upload/") && ok;
    std::string raw_filename = raw_res._headers["Location"].substr(raw_res._headers["Location"].find_last_of('/') + 1);
    ok = expect_eq("upload raw file body", read_file_content(upload_dir + "/" + raw_filename), raw_body) && ok;

    const std::string octet_body = "raw-octet-body";
    std::stringstream octet_len;
    octet_len << octet_body.size();
    const std::string octet_head =
        "POST /upload HTTP/1.1\r\n"
        "Host: example.com\r\n"
        "Content-Type: application/octet-stream\r\n"
        "Content-Disposition: attachment; filename=\"octet_name.bin\"\r\n"
        "Content-Length: " + octet_len.str() + "\r\n"
        "\r\n";
    std::pair<types::HttpStatus, Request> octet_req = parse_with_connection(octet_head, octet_body);
    Response::_http_response octet_res = Response::make_response(octet_req, upload_location, 8080);
    ok = expect_eq("upload octet status", octet_res._status, static_cast<uint16_t>(201)) && ok;
    ok = expect_eq("upload octet location", octet_res._headers["Location"], "/upload/octet_name.bin") && ok;
    ok = expect_eq("upload octet file body", read_file_content(upload_dir + "/octet_name.bin"), octet_body) && ok;

    const std::string bad_multipart_head =
        "POST /upload HTTP/1.1\r\n"
        "Host: example.com\r\n"
        "Content-Type: multipart/form-data\r\n"
        "Content-Length: 4\r\n"
        "\r\n";
    std::pair<types::HttpStatus, Request> bad_upload_req = parse_with_connection(bad_multipart_head, "test");
    Response::_http_response bad_upload_res = Response::make_response(bad_upload_req, upload_location, 8080);
    ok = expect_eq("upload bad multipart status", bad_upload_res._status, static_cast<uint16_t>(400)) && ok;

    std::vector<std::string> upload_files;
    upload_files.push_back("sample.txt");
    upload_files.push_back("octet_name.bin");
    if (!raw_filename.empty())
        upload_files.push_back(raw_filename);
    cleanup_upload_dir(upload_dir, upload_files);
