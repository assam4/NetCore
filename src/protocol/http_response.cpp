...
namespace http {
	namespace core {
		...
		void Response::build_response(_http_response& res, const Request& req, types::HttpStatus parse_status, const types::__location& location, uint16_t server_port) {
			if (handle_parse_error(res, req, parse_status, location)) return;
			if (handle_method_check(res, req, location)) return;
			if (handle_redirect(res, req, location)) return;
			if (handle_upload(res, req, location)) return;
			if (handle_delete(res, req, location)) return;
			std::string fs_path = resolve_path(location.content.root, req.start_line.uri);
			struct stat st;
			if (handle_stat(res, req, location, fs_path, st)) return;
			if (handle_directory(res, req, location, fs_path, st)) return;
			if (handle_cgi(res, req, location, fs_path, server_port)) return;
		...
		Response::_http_response Response::make_response(const std::pair<types::HttpStatus, Request>& status_req, const types::__location& location, uint16_t server_port) {
			...
			build_response(res, req, parse_status, location, server_port);
			if (req.start_line.method == types::HEAD)
				res._body.clear();
			return res;
		}
