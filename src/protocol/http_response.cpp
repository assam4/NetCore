...
namespace http
{
	namespace core
	{
		...
		{
			// Try the standard way first (in the current dir)
		...
		void Response::set_body_length_field(_http_response &res)
		{
			...
			bool no_message_body = (res._status >= types::CONTINUE && res._status < types::OK) || res._status == types::NO_CONTENT || res._status == types::NOT_MODIFIED;
			if (no_message_body)
			{
		...
		{
			if (req.start_line.method == types::POST && !location.cgi_extension.empty())
			{
				size_t dot = req.start_line.uri.find_last_of('.');
				if (dot != std::string::npos)
				{
					std::string ext = req.start_line.uri.substr(dot);
		...
		bool Response::handle_directory(_http_response &res, const Request &req, const types::__location &location, std::string &fs_path, struct stat &st)
		{
			...
			if (uri.empty() || (uri[uri.size() - 1] != '/' && uri != "/directory"))
			{
				bool should_redirect = true;
				// Hack for tester /directory path without slash
				if (uri == "/directory")
				{
		...
		{
			if (req.start_line.method != types::POST)
				return false;
