...
namespace http {
	namespace core  {
		...
		const types::__location& get_best_location(const VirtualHost& vhost, const std::string& uri_path) {
		...
		std::string find_error_page(types::HttpStatus status, const std::map<uint16_t, std::string>& pages) {
			std::map<uint16_t, std::string>::const_iterator it = pages.find(status);
			if (it != pages.end())
				return it->second;
			return "/error/" + to_string(status) + ".html";
		}

		types::HttpStatus validate_location_rules(const Request& req, const types::__location& loc) {
