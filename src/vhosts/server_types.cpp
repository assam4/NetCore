	...
	namespace   core {
		namespace   types {
			...
			void    __content::fill_allowed_methods(const std::set<std::string>& data) {
				allowed_methods = 0;
				if (data.empty())
					allowed_methods = GET | POST | DEL | HEAD;
				else
			...
			void    __location::fill_upload_location(const std::string& data) {
			 ...
			 void    __location::fill_cgi_extension(const std::vector<std::string>& data) {
				for (size_t i = 0; i < data.size(); ++i) {
					const std::string& extension = data[i];
					if (extension.empty())
						continue;
					if (extension[0] != '.')
						throw std::runtime_error("Parsing error: Incorrect cgi extension: '" + extension + "'.\n");

					// Interpreter path is required
					if (i + 1 >= data.size() || data[i + 1].empty())
						throw std::runtime_error("Parsing error: Missing interpreter for cgi extension: '" + extension + "'. Must provide absolute path.\n");

					const std::string& interpreter = data[i + 1];

					// Interpreter path must be absolute
					if (interpreter[0] != '/')
						throw std::runtime_error("Parsing error: CGI interpreter path must be absolute for extension '" + extension + "': '" + interpreter + "'.\n");

					cgi_extension[extension] = interpreter;
					++i;
				}
			}
		}
