    ...
    namespace core {
        ...
        VirtualHost& VirtualHost::set_listen(const std::set<std::string>& data, const std::vector<VirtualHost>& servers) {
            for (std::set<std::string>::const_iterator it = data.begin(); it != data.end(); ++it) {
                ...
                std::string host_part = field.substr(0, colon != std::string::npos ? colon : field.size());
                std::string port_part = (colon != std::string::npos) ? field.substr(colon) : "";
                if (host_part == "localhost") {
                    listen.insert(types::__listen().fill("127.0.0.1" + port_part, servers));
                    listen.insert(types::__listen().fill("[::1]" + port_part, servers));
                } else {
                    listen.insert(types::__listen().fill(field, servers));
                }
            }
        return *this;
        }
