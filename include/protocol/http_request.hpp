...
namespace http
{
	namespace core
	{
		...
		const static size_t DefaultMaxBodyLength = 10485760000; // 10MB
		...
		struct __start_line
		{
			...
			{
				if (line.empty() || line.length() > StartLineLength || line.find_first_of("\r\n\t") != std::string::npos)
			...
			{
				is_head = false;
				if (line.compare(start, end, "GET") == 0)
					method = types::GET;
				...
				else if (line.compare(start, end, "POST") == 0)
					method = types::POST;
				else if (line.compare(start, end, "DELETE") == 0)
					method = types::DEL;
				else
